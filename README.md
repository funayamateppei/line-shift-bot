...
| GAS ウェブアプリ（日付を選ぶ画面） | 無料 | リクエスト数の上限なし | 月 40 回ほど、合計 30 秒ほど |
...
- **日付を選ぶのは LINE のカードではなく、GAS のウェブアプリの画面**。Flex Message は送ったあとに書き換えられないので、カードの上で選ばせると 1 タップごとにカードを送り直すことになり、トークがカレンダーで埋まる。ホスティングは要らない（webhook のために公開済みの URL をそのまま使う）ので、費用は変わらない
...
貼ったあとは**ウェブアプリとしてデプロイする**（webhook と画面が同じ URL）。直すときは
「新しいデプロイ」ではなく**既存デプロイの編集**にすること。新しく作ると URL が変わり、
メンバーに配ってある画面の URL がすべて死ぬ。
