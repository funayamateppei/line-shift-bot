 ...
 *
 * ヘッダが読めないので署名の検証はしない。代わりに URL へ合言葉を付ける
 * （設定シートの「webhook合言葉」。setup() が作る）。
 *
 * カレンダーの画面（12_web.gs）を配るようになって、ウェブアプリの URL は
 * メンバー全員の知るところになった。「URL を知っているのは LINE と管理者だけ」
 * という前提はもう使えないので、合言葉を知らない POST は受け取らない。
 */
...
/**
 * この POST を受け取ってよいか。
 * LINE に登録した URL の ?w= が、設定シートの合言葉と一致するときだけ通す。
 *
 * 空にすると誰でも通る。合言葉を貼り違えて Bot が黙ったときの逃げ道として
 * 残してある（セルを消せば、再デプロイなしですぐ元に戻る）。
 */
